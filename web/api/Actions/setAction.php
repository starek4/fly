<?php
    ...
    try
    {
        ...
        $result = $deviceRepo->SetActionPending($device_id, $action);
    }
    catch (Exception $e)
    {
        $response["Success"] = false;
        ...
        echo json_encode($response, JSON_PRETTY_PRINT);
        $logger->Error("Exception when setting action state: " . $e->getMessage());
    }
    ...
    $response["Error"] = "";
    echo json_encode($response, JSON_PRETTY_PRINT);
    $logger->Info("Device set action state successfully: " . $device_id);
?>
