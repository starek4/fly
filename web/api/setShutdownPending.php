<?php
    ...
    try
    {
        ...
        $result = $deviceRepo->SetShutdownPending($device_id);
    }
    catch (Exception $e)
    {
        $response["Success"] = false;
        ...
        echo json_encode($response, JSON_PRETTY_PRINT);
        $logger->Error("Exception when setting shutdown state: " . $e->getMessage());
    }

    $response["Success"] = true;
    $response["Error"] = "";
    echo json_encode($response, JSON_PRETTY_PRINT);
    $logger->Info("Device set shutdown state successfully: " . $device_id);
?>