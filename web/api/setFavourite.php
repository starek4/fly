<?php
    ...
    try
    {
        ...
        $result = $deviceRepo->SetFavourite($device_id);
    }
    catch (Exception $e)
    {
        $response["Success"] = false;
        ...
        echo json_encode($response, JSON_PRETTY_PRINT);
        $logger->Error("Exception when setting favourite: " . $e->getMessage());
    }

    $response["Success"] = true;
    $response["Error"] = "";
    echo json_encode($response, JSON_PRETTY_PRINT);
    $logger->Info("Device set favourite successfully: " . $device_id);
?>